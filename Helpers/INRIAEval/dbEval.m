...
stra={res(1,:).stra}; stre={res(:,1).stre}; scores1=round(scores*1000)/10;
...
for p=1:nPlots
  ...
  if( plotAlg )
    ...
    for g=1:nGt, lgd1{g}=sprintf('%.1f%% %s',scores1(g,p),stre{g}); end
    colors1=uniqueColors(1,max(10,nGt)); styles1=repmat({'-','--'},1,nGt);
  else
    xs1=xs(p,:); ys1=ys(p,:); fName1=[fName stre{p}]; lgd1=stra;
    for d=1:nDt, lgd1{d}=sprintf('%.1f%% %s',scores1(p,d),stra{d}); end
